...
IndexBuilder_build(benchmark::State& state) {
    ...
    std::tie(type_params, index_params) = ivfpq_generate_params(index_type, metric_type, nlist, nbits, m);
...
CustomArguments(benchmark::internal::Benchmark* b) {
    ...
    for (int nlist = 1024; nlist <= 65536; nlist *= 2) {
        for (int nbits = 1; nbits <= 16; nbits++) {
            for (int m = 2; DIM % m == 0; m++) {
                b->Args({nlist, nbits, m});
            }
        }
    }
}
