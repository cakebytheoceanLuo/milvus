...
//BENCHMARK(IndexBuilder_build)->Name("Build: IVF_SQ8/L2/VectorFloat")->Apply(CustomArguments);

static void
IVFSQ8_search(benchmark::State& state) {
    auto index_type = milvus::knowhere::IndexEnum::INDEX_FAISS_IVFSQ8;
    auto metric_type = milvus::knowhere::Metric::L2;
...
//static void
//CustomArguments(benchmark::internal::Benchmark* b) {
//    for (int nlist = 64; nlist <= 2048; nlist *= 2) {
...
static void
CustomArguments2(benchmark::internal::Benchmark* b) {
    static std::array<int, 4> nbits_arr{4, 6, 8, 16};
    for (int nlist = 1024; nlist <= 65536; nlist *= 2) {
        for (auto nbits : nbits_arr) {
            for (int nprobe = 1; nprobe <= nlist; nprobe *= 2) {
...
BENCHMARK(IVFSQ8_search)->Name("Search: IVFSQ8/L2/VectorFloat")->Apply(CustomArguments2);